...
async function bundle(options = {}) {
  ...
  if (!staticDir && !edgeEntry && !serverlessEntry) {
    throw new Error("Must provide at least one of staticDir, edgeEntry, or serverlessEntry");
  }
  ...
  if (edgeEntry) {
    await bundleEdgeFunction(edgeEntry, outputDir + "/functions/_edge.func", manipulateEsbuildOptions);
    await fs.promises.writeFile(outputDir + "/functions/_edge.func/.vc-config.json", JSON.stringify({
      runtime: "edge",
      entrypoint: "index.js"
    }, null, 2));
  }
  if (serverlessEntry) {
    await bundleServerlessFunction(serverlessEntry, outputDir + "/functions/index.func", manipulateEsbuildOptions);
    await fs.promises.writeFile(outputDir + "/functions/index.func/.vc-config.json", JSON.stringify({
      runtime: "nodejs16.x",
      handler: "index.js",
      launcherType: "Nodejs"
    }, null, 2));
...
async function createConfigFile(outputDir, options) {
  ...
  await fs.promises.writeFile(outputDir + "/config.json", JSON.stringify(config, null, 2));
}
