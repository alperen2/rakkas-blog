...
function sortRoutes(routes) {
  const processedRoutes1 = routes.map((route) => {
    ...
    return {
      original: route,
      dynamicCount: ((_a = route[0].match(/\[/g)) == null ? void 0 : _a.length) || 0,
...
function compareSegments(a, b) {
  ...
  if (definedOrder)
    return definedOrder;
  return a.paramCount - b.paramCount || a.content.localeCompare(b.content);
}
...
function apiRoutes() {
  ...
  async function generateRoutesModule() {
    ...
    const middlewareFiles = await (await glob(root + middlewarePattern)).sort((a, b) => b.length - a.length);
    const middlewareDirs = middlewareFiles.map(
      (f, i) => [i, path3.dirname(f)]
    );
...
`;
    ...
    const endpointRoutes = sortRoutes(
      endpointFiles.map((endpointFile, i) => {
        const relName = path3.relative(root, endpointFile);
...
function pageRoutes(options = {}) {
  ...
  async function generateRoutesModule(client) {
    const pageFiles = (await glob2(routesRoot + pagePattern)).map(
      (f) => path4.relative(resolvedConfig.root, f).replace(/\\/g, "/")
    );
    let pageImporters = "";
...
`;
    ...
    const layoutFiles = (await glob2(routesRoot + layoutPattern)).sort((a, b) => b.length - a.length).map((f) => path4.relative(resolvedConfig.root, f).replace(/\\/g, "/"));
    const layoutDirs = layoutFiles.map((f) => path4.dirname(f));
    let layoutImporters = "";
...
`;
    ...
    const guardFiles = (await glob2(routesRoot + guardPattern)).sort((a, b) => a.length - b.length).map((f) => path4.relative(resolvedConfig.root, f).replace(/\\/g, "/"));
    const guardDirs = guardFiles.map((f) => path4.dirname(f));
    let guardImporters = "";
...
`;
    ...
    const singlePageGuardFiles = (await glob2(routesRoot + singlePageGuardPattern)).map((f) => path4.relative(resolvedConfig.root, f).replace(/\\/g, "/"));
...
`;
    ...
    const pageRoutes2 = sortRoutes(
      pageFiles.map((endpointFile, i) => {
        const relName = path4.relative(routesRoot, endpointFile).replace(/\\/g, "/");
  ...
  return [
    {
      ...
      configResolved(config) {
        ...
        isLayout = micromatch2.matcher(
          path4.resolve(routesRoot + "/" + layoutPattern).replace(/\\/g, "/")
        );
        isPage = micromatch2.matcher(
          path4.resolve(routesRoot + "/" + pagePattern).replace(/\\/g, "/")
        );
        isGuard = micromatch2.matcher(
          path4.resolve(routesRoot + "/" + guardPattern).replace(/\\/g, "/")
        );
        isSinglePageGuard = micromatch2.matcher(
          path4.resolve(routesRoot + "/" + singlePageGuardPattern).replace(/\\/g, "/")
        );
...
// src/features/run-server-side/implementation/transform-server-side.ts
import * as t2 from "@babel/types";

// src/features/run-server-side/implementation/transform-utils.ts
...
function isRunServerSideCall(expr, nameRef) {
  ...
  if (t.isIdentifier(callee)) {
    ...
    nameRef.name = (_b = (_a = binding == null ? void 0 : binding.path.node) == null ? void 0 : _a.imported) == null ? void 0 : _b.name;
    ...
    nameRef.name = (_c = callee.property) == null ? void 0 : _c.name;
...
function removeUnreferenced(program, alreadyUnreferenced) {
  for (; ; ) {
    ...
    for (const [name, binding] of Object.entries(program.scope.bindings)) {
      ...
      if ((parent == null ? void 0 : parent.isImportDeclaration()) && parent.node.specifiers.length === 1) {
        parent.remove();
      } else {
        binding.path.remove();
      }
                ...
                body.traverse({
                  Identifier: {
                    exit(identifier3) {
                      ...
                      if ((_a = program.scope.getBinding(identifier3.node.name)) == null ? void 0 : _a.referencePaths.includes(identifier3)) {
                        return;
                      }
                      if ((binding == null ? void 0 : binding.path.get("id")) === identifier3 || (binding == null ? void 0 : binding.referencePaths.includes(identifier3))) {
...
// src/features/run-server-side/implementation/transform-client-side.ts
    ...
    },
    {
      ...
      async transform(code, id, options) {
        ...
        if (id.startsWith(resolvedConfig.root) && code.match(
          ...
          plugins.push(
            (options == null ? void 0 : options.ssr) ? babelTransformServerSideHooks(moduleId) : babelTransformClientSideHooks(moduleId, ref)
          );
        }
        if (!plugins.length) {
          return;
...
var adapters = {
  ...
  "cloudflare-workers": {
    ...
    async bundle(root) {
      ...
      if (!entry) {
        entry = path5.resolve(root, "dist/server/entry-cloudflare-workers.js");
        await fs2.promises.writeFile(entry, CLOUDFLARE_WORKERS_ENTRY);
      }
      cloudflareWorkers(
        {
          output: path5.resolve(
            root,
  ...
  vercel: {
    ...
    async bundle(root) {
      ...
      vercel({
        ...
        staticDir: path5.resolve(root, "dist/client"),
  ...
  "vercel-edge": {
    ...
    async bundle(root) {
      ...
      if (!entry) {
        entry = path5.resolve(root, "dist/server/entry-vercel-edge.js");
  ...
  netlify: {
    ...
    async bundle(root) {
      ...
      if (!entry) {
        entry = path5.resolve(root, "dist/server/entry-netlify.js");
  ...
  "netlify-edge": {
    ...
    async bundle(root) {
      ...
      if (!entry) {
        entry = path5.resolve(root, "dist/server/entry-netlify-edge.js");
  ...
  deno: {
    ...
    async bundle(root) {
      ...
      if (!input) {
        input = path5.resolve(root, "dist/server/entry-deno.js");
      ...
      deno(
        {
          input,
          output: path5.resolve(root, "dist/deno/mod.js"),
...
function findEntry(root, name) {
  const entries = [
    path5.resolve(root, name) + ".ts",
    path5.resolve(root, name) + ".js",
    path5.resolve(root, name) + ".tsx",
    path5.resolve(root, name) + ".jsx"
  ];
  return entries.find((entry) => fs2.existsSync(entry));
}
async function generateStaticAssetManifest(root) {
  const files = walk(path5.resolve(root, "dist/client"));
  await fs2.promises.writeFile(
    path5.resolve(root, "dist/server/static-manifest.js"),
...
function walk(dir, root = dir, entries = /* @__PURE__ */ new Set()) {
  ...
  for (const file of files) {
    ...
    if (stat.isDirectory()) {
      ...
      entries.add("/" + path5.relative(root, filepath).replace(/\\/g, "/"));
    }
  }
  return entries;
}
...
// src/features/run-server-side/implementation/transform-client-page.ts
        ...
        exit(program) {
          ...
          program.traverse({
            ExportNamedDeclaration: {
              enter(path6) {
                if (t4.isFunctionDeclaration(path6.node.declaration) && SSR_EXPORTS.includes(path6.node.declaration.id.name)) {
                  path6.remove();
                  modified = true;
                } else if (t4.isVariableDeclaration(path6.node.declaration)) {
                  const declarations = path6.get("declaration").get("declarations");
                ...
                } else if (path6.node.specifiers.length) {
...
function rakkas(options = {}) {
  ...
  return [
    ...
    ...react({
      ...
      babel(id, opts) {
        var _a, _b;
        const inputOptions = typeof ((_a = options.react) == null ? void 0 : _a.babel) === "function" ? options.react.babel(id, opts) : (_b = options.react) == null ? void 0 : _b.babel;
        if (!(opts == null ? void 0 : opts.ssr) && (resolvedConfig.api.rakkas.isPage(id) || resolvedConfig.api.rakkas.isLayout(id))) {
          return {
            ...inputOptions,
            plugins: [
              babelTransformClientSidePages(),
              ...(inputOptions == null ? void 0 : inputOptions.plugins) ?? []
            ]
          };
        } else {
          return inputOptions || {};
...
${error.stack}`), {
  ...
  try {
    ...
    info(
      `
  ${pico.green(
        pico.black(pico.bgMagenta(" RAKKAS ")) + " " + pico.magenta(version) + " development server is running \u{1F483}"
